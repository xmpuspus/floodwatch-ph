...
These outputs are joined with WorldPop population estimates and Microsoft GlobalML building footprint counts at the barangay level to produce exposure figures, and compared with official UP NOAH / PAGASA / MGB hazard maps to produce the civic gap layer.
...
| WorldPop population | WorldPop, CC-BY-4.0 | Modeled aggregate; no individual identifiers | Published as rounded barangay-level totals only (nearest 10). |
| Microsoft GlobalML building footprints | Microsoft, ODbL-equivalent | Indirect: building outline on a public map | Published as barangay-level count only; no per-building attributes. |
| Barangay boundaries | PSA (public) / OSM (ODbL) | Administrative; public | Published as click-target polygons with aggregate properties only. |
| Official hazard map extents | UP NOAH / PAGASA / MGB (government public) | None | Published as a boolean overlay per barangay. |
...
| Recurrence score (Track B output) | Computed from AlphaEarth | None on its own: a probability number on a 10-30 m sample grid | Published as a point grid. Points are sample-grid centers, not buildings. |
| Barangay exposure aggregate | Computed join | Barangay aggregate (population, building count, flood share) | Published; all figures are aggregate, not per-household. |
...
- A recurrence-score point on a 10-30 m grid does not name a person.
- A barangay-level population count or building count does not name a person.
- The "put together with other information" clause is the legal exposure point: combining a barangay-level flooded count with a barangay household registry could identify a household's flood status in a very small barangay. This is a lower exposure than SolarMap.PH's per-building solar coordinates because FloodWatch publishes only counts and polygon extents, never per-dwelling geometry.
- **Mitigation:** All exposure figures are aggregated to barangay level with rounding. No per-building flood-status geometry is published. No per-dwelling property key appears in any output. CI gate `scripts/check_no_pii.py` enforces this on every release.

### §3(c) "consent"

Public satellite data (Sentinel-1, AlphaEarth) and public administrative boundaries (PSA, OSM) do not require individual consent under the public-records doctrine inherent to RA 10173's coverage. FloodWatch does not capture, store, or republish raw imagery. Published outputs are derived statistical indicators. The publication boundary (barangay aggregation, takedown channel, attribution chain) is the responsibility-shifting mechanism in lieu of per-subject consent.
...
- **Proportionality:** Only the aggregate barangay-level figures necessary for the research question are published. No per-dwelling geometry, no household attributes.
...
| Re-identification by combining barangay aggregate with a household registry | Low (counts are aggregate, not addresses) | Medium | Barangay counts are rounded; no property key links a row to a specific dwelling; conservative language ("barangay-level aggregate"). |
...
| Hazard gap layer | `site/public/data/hazard_gap.geojson` | Republished with each update | Barangay aggregate; disclaimer in `_meta.disclaimer`. |
| Barangay exposure aggregate | `site/public/data/barangay_exposure.json` | Republished with each update | Rounded counts; no household resolution. |
...
FloodWatch.PH publishes statistical indicators derived from public satellite data and public administrative datasets. The publication boundary -- barangay aggregation, rounding, no per-dwelling geometry, CI-enforced PII gate, conservative civic language, and a published takedown channel -- is, in the author's good-faith judgment, sufficient to satisfy the proportionality and transparency principles of RA 10173 for an inaugural launch. The environmental data processed here carries a lower per-person exposure than SolarMap.PH's per-building rooftop geometry. The PIA will be re-reviewed at each subsequent release, and a formal NPC advisory opinion will be sought in the post-launch quarter as scale or scope warrants.
